...
export async function PATCH(request: NextRequest, ctx: RouteContext<"/api/[libraryCode]/copies/[id]">) {
...
export async function DELETE(_request: NextRequest, ctx: RouteContext<"/api/[libraryCode]/copies/[id]">) {
  const { libraryCode, id } = await ctx.params;
  const { context, response } = await requireLibraryContext(libraryCode, { require: "edit" });
  if (!context) return response;
