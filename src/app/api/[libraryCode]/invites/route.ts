...
export async function GET(_request: NextRequest, { params }: RouteContext<"/api/[libraryCode]/invites">) {
 ...
 * anyway).
 */
export async function POST(request: NextRequest, { params }: RouteContext<"/api/[libraryCode]/invites">) {
  const { libraryCode } = await params;
  const { context, response } = await requireLibraryContext(libraryCode, { require: "manage" });
  if (!context) return response;
