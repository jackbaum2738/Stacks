...
export async function PATCH(request: NextRequest, ctx: RouteContext<"/api/people/[id]">) {
 ...
 */
export async function DELETE(_request: NextRequest, ctx: RouteContext<"/api/people/[id]">) {
  const { context, response } = await requireLibraryContext({ require: "edit" });
  if (!context) return response;
  const { id } = await ctx.params;
